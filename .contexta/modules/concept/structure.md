...
这个名字用于避免把成组、分位、对照、承接或分叉问题误写成单个 concept 定义、policy 约束、template 骨架或 example 样本。

## Definition

组织语言，用于表达多个语义单元如何在同一目的下成组、分位、对照、承接或分叉。
...
| [[example]] | example 提供样本；structure 可以表达多个 sample 在同一教学目的下如何成组。 |
...
| architecture | architecture 可以被 structure 表达，但当前不先展开完整结构形态分类。 |
| branch | branch 可以是 structure 中的分叉关系，但当前不先升级为独立结构形态。 |
...
- [[example]]：example 可以用 structure 表达多个 sample 在同一教学目的下的组织方式。
...
workflow、pipeline、architecture 和 branch 可以被 structure 表达，但它们不是当前 definition 的起点。下一步先从这些 subtype 入手，再反推 structure 的稳定定义。
