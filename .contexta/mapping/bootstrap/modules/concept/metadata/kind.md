...
contexta 需要一个名字表示 metadata 中选择内容语言的入口字段。
...
当 `kind: signal` 出现在 `.contexta/mapping/bootstrap/modules/signal/*.md` 中时，它表示当前 module 使用 signal definition language。
...
| Neighbor | Difference |
| --- | --- |
| [[mapping/bootstrap/modules/concept/metadata|metadata]] | metadata 是 frontmatter 字段层；kind 是其中负责选择 content language 的字段。 |
...
| [[mapping/bootstrap/modules/concept/signal|signal]] | signal 可以作为 content language entry，用于 signal definition module；signal instance 不作为长期 module kind。 |
...
如果这出现在 `.contexta/mapping/bootstrap/modules/signal/*.md` 中，它表示当前 module 定义一个 signal warning type。

这不是未来 CLI 输出的 signal instance。
