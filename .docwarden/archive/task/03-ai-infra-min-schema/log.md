...
已将 `index.md`、`plan.md`、`schema-scope.md` 和 `log.md` 的正文改成中文。保留 frontmatter key、状态值、文件名和必要技术名词。
...
- 当前 workflow 只应该在自己的 workspace 下做事。对本任务而言，working material 只放在 `.docwarden/task/`。
...
- 工作面命名采用“编号 + 短语义 slug + `index.md` 查询增强”的方案。
- 命名规则可以派生为一条 policy，并允许 subagent 起草。
- `index.md` 是每个工作面的 lead file，是该工作面的唯一震源；核心描述、规约和目录散发关系以它为准。
- `log.md` 只负责按时间记录事件。
- working 状态建模还需要继续讨论，尤其是是否通过扩展文件结构或引入必要状态来解决。

## [2026-05-12] loop-2 | 状态建模 review 通过

sayori 选择 C 方案：文件 frontmatter 带最小状态，`index.md` 维护文件索引。
...
暂不引入工作面或文件级 `superseded`。替代关系先通过 `index.md` 和 `log.md` 的自然语言记录。
...
已将 `operation-rules.md` 标记为 accepted，并更新 `index.md` 与 `plan.md`。
...
- pick 产物后续需要规划长期文件夹结构，不能放在 `.docwarden/task/` 中作为长期材料，因为 working 的设计是短命的。
...
已将 `promote-pick-boundary.md` 标记为 accepted，并更新 `index.md` 与 `plan.md`。
...
已将 `acceptance.md` 标记为 accepted，并将 `index.md` 的 `workspace_status` 更新为 `closed`。
