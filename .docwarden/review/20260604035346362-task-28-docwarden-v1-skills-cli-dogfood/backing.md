...
### Objective
- 实现 docwarden v1 最小可运行的 skills + CLI dogfood 形态。

### Boundary
- 可以修改 `apps/docwarden` CLI 与测试。
- 可以修改 `.agents/skills/dw-*` repo-local Codex skills。
...
- [2026-06-04] implemented repo-local runtime skill baseline: expanded `dw-review-doc`, added `dw-promote`, and updated `dw-pick` to use `rtk pnpm exec docwarden`.
...
- [2026-06-04] validation passed: quick_validate for `dw-review-doc`, `dw-promote`, and `dw-pick`; `apps/docwarden` typecheck, build, test, and smoke.
