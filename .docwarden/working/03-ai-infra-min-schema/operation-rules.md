...
本文件记录 Loop 3 已通过 review 的 working 定义，用于定义当前项目中 agent 对 `.docwarden/working/` 和 `docs/` 的操作边界。
...
agent 可以在 `.docwarden/working/` 下自动执行这些动作：

- 创建新的工作面目录。
- 创建或修改工作面的 `roadmap.md`。
- 创建或追加工作面的 `log.md`。
- 创建或修改该工作面内的草案文件。
- 更新 `roadmap.md` 的文件索引。
...
- 修改 `docs/` 下的文件。
- 将 `.docwarden/working/` 中的内容直接移动到 `docs/`。
