...
export const seedVendor = 'contexta'
export const seedRef = 'contexta-seed-v0'
...
concept is a named semantic object in local contexta. It defines what an object is and why the name is needed.
`,
  },
  {
...
signal definition is local contexta material with \`kind: signal\`. It names a possible semantic drift, lists trigger surface, and points to basis material.
...
primitive-creator is the local primitive that creates new contexta language primitives from semantic model material.
...
1. Parse md surface.
2. Run recognition from local contexta material.
