...
describe('isomorph import boundaries', () => {
  ...
  it('keeps runtime as an aggregation layer instead of application orchestration', async () => {
    ...
    expect(runtime).not.toContain('loadIsomorphModelEffect')
  ...
  it('keeps CLI exit code mapping out of isomorph error types', async () => {
  ...
  it('keeps package root from exporting local model internals', async () => {
    ...
    expect(index).not.toContain('IsomorphModel')
    expect(index).not.toContain('IsomorphDocument')
