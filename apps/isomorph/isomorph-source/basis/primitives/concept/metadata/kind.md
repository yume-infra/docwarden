...
当 `kind: signal` 出现在 `basis/lint/signal/*.md` 中时，它表示当前 module 使用 signal definition language。

`kind: signal` 不表示 signal instance。

当 `kind: magic-word` 出现在 `basis/grammars/magic-word/*.md` 中时，它表示当前 module 使用 magic-word registry language。

当 `kind: locator` 出现在 `basis/grammars/locator/*.md` 中时，它表示当前 module 使用 locator instance language。
