...
- isomorph 需要创建或修改 example 内容、章节或教学材料。
- isomorph 需要判断一个 example 是否足以教 agent 理解、书写或判断。
- isomorph 需要维护 example 章节中的例子质量。
...
用户要求 agent “把 template 规则写进 `apps/isomorph/isomorph-source/bootstrap/contexta/templates/user-context.md`”。
...
```md
这条内容是 policy，不是 template 骨架。template 只能提供复制后的初始结构；template 的边界规则应进入 `apps/isomorph/isomorph-source/basis/grammars/policy/template-boundary.md`。
```
