...
- isomorph 需要创建或修改 template。
- isomorph 需要判断某个内容应进入 template、module 还是 policy。
- isomorph 需要在 template 中提供 assertion 或 module 的书写槽位。
- isomorph 需要判断某个 template 是否会反向制造 kind。
...
如果 template 同时承接规则本体、概念定义或生命周期，isomorph 会把内容格式协议和 docwarden 操作流程混在一起。
...
agent 准备把 template 边界规则写进 `apps/isomorph/isomorph-source/basis/templates/policy.md`。
...
```md
这是真实规则，应进入 `apps/isomorph/isomorph-source/basis/grammars/policy/template-boundary.md`。template 只能保留占位骨架。
```
