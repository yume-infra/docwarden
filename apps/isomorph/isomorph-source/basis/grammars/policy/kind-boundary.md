...
- [[primitives/concept/relation|relation]]

适用条件：

- isomorph 需要读取或创建 md module frontmatter。
- isomorph 需要判断某个词是否可以作为 `kind`。
- isomorph 需要区分 content metadata 与 docwarden operation metadata。
- isomorph 需要处理 template 文件中 `kind` 对复制后目标 module 的指向。
...
- isomorph MUST NOT 用 `sub_type`、`structure_type` 或等价 metadata field 表达 structure 关系。
- structure language 之间的上位关系 SHOULD 由 [[primitives/concept/relation|relation]] 承接。
- template 文件中的 `kind` MAY 指向复制后目标 module 的 content kind。
- template 文件中的 `kind` MUST NOT 被解释为 template artifact 自身的 kind。
- `status / created / updated / owner` MUST 被视为 docwarden operation metadata，而不是 isomorph content kind。
...
isomorph 的理论关系应通过 concept、relation、policy 和 example 维护，不应塞进 frontmatter subtype 树。
...
在 `apps/isomorph/isomorph-source/basis/templates/policy.md` 中，这个字段暂时指向复制后目标 module 的 kind。是否改成 `target_kind` 属于 template loop，不在本 policy 中直接修改。
