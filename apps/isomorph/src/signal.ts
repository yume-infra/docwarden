import type { IsomorphDiagnostic, LintSignal, MarkdownSurface, RecognitionResult, SignalCandidate, SignalDefinition, TriggerHit } from './domain.js'
...
export function evaluateSignal(signal: SignalDefinition, surface: MarkdownSurface, recognition: Pick<RecognitionResult, 'recognizedRole'>): SignalCandidate {
  ...
  return {
    signal: signal.id,
    isomorphPath: signal.isomorphPath,
    applicable,
    matched,
    total,
    triggerHits,
...
function signalDiagnostics(signal: SignalDefinition, triggerHits: readonly TriggerHit[]): readonly IsomorphDiagnostic[] {
  const diagnostics: IsomorphDiagnostic[] = []
