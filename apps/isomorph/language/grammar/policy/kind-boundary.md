...
- structure language 之间的上位关系 SHOULD 由 [[language/primitive/concept/relation|relation]] 承接。
...
在 `apps/isomorph/language/grammar/template/policy.md` 中，这个字段暂时指向复制后目标 module 的 kind。是否改成 `target_kind` 属于 template loop，不在本 policy 中直接修改。
