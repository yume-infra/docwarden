...
- isomorph 需要定义 semantic-lint signal。
- isomorph 需要判断 trigger 是否可以作为 lint 条件。
- isomorph 需要让 lint signal 指向 locator marker。
- isomorph 需要区分 signal 与 review judgment。
...
- module path 与 heading MAY 作为 signal context，但不是 final assertion locator。
...
locator: [[language/primitive/concept/assertion#^def-1|assertion definition]]
```
