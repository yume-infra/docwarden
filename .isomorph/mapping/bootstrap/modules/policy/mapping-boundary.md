...
本 policy 用于避免 isomorph 的 bootstrap 内容与 isomorph 到 docwarden 的映射内容被压平到同一层。
...
- isomorph 需要判断某个内容属于 bootstrap mapping 还是 docwarden mapping。
- isomorph 需要说明 docwarden 资产如何使用 isomorph 定义的内容语言。
...
- isomorph 的 bootstrap 内容 MUST 放在 `.isomorph/mapping/bootstrap/` 下。
- isomorph 面向 docwarden 资产的内容语言定义和 template MUST 放在 `.isomorph/mapping/docwarden/` 下。
...
如果 `.isomorph` 只按 `modules/concept`、`modules/policy`、`templates` 分层，目录只能表达 content kind，不能表达 bootstrap mapping 和 docwarden mapping 的差异。
...
```md
`.isomorph/mapping/bootstrap/modules/concept/user-context.md`
```

这会把 docwarden 资产映射误放成 isomorph 的 bootstrap concept。

### Borderline

```md
`.isomorph/mapping/bootstrap/modules/concept/template.md`
```
