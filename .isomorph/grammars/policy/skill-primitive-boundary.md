...
This policy keeps skill primitive as an agent-use contract shape, distinct from bootstrap primitives and final runtime artifacts.
...
- skill primitive MUST model review gates as Workflow moves, and SHOULD use [[primitives/concept/lead-review|lead-review]] when user review is needed.
...
- root `.isomorph/primitives` MUST NOT become canonical storage for concrete skill contracts.
...
- skill primitive MUST NOT directly materialize final SKILL.md artifacts from root bootstrap material.
- skill primitive MUST NOT 把 legacy headings `Drift Pressure / Intervention / Activation / Judgment Surface / Deterministic Boundary / Review Gate` 当作 exportable surface。
