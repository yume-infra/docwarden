...
用户要求 agent “把 template 规则写进 `.isomorph/exports/contexta/templates/user-context.md`”。
...
```md
这条内容是 policy，不是 template 骨架。template 只能提供复制后的初始结构；template 的边界规则应进入 `.isomorph/grammars/modules/policy/template-boundary.md`。
```
