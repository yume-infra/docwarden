...
定义 isomorph 中断言与 md module 的语义粒度。
...
- isomorph 需要拆分、命名、组合或 review module。
- isomorph 需要组织 assertion 在 module 内部的语义粒度。
...
- isomorph MUST 将断言视为最小语义审查单位。
- isomorph MUST 将 md module 视为默认组合单位。
...
- isomorph SHOULD NOT 为断言默认创建独立文件或独立存储位置。

## Rationale

本 policy 已通过 `Applies to` 指向 [[primitives/modules/concept/assertion|assertion]] 和 [[primitives/modules/concept/module|module]]。
...
- isomorph 文档作者 MUST 在写作前明确目标读者。
- isomorph 文档作者 MUST 在写作前明确读者目标。
