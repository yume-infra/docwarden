...
- [[primitives/modules/concept/naming|naming]]
...
- isomorph internal link MUST NOT use Markdown link syntax as the first internal expression.
...
目标落点是 `primitives/modules/concept/structure/pipeline.md`。
...
```md
[[primitives/modules/concept/structure/pipeline#Definition|pipeline#Definition]]
```

这是 section link，可以用于 Read next。它稳定到 heading，但还不是 assertion locator。

```md
[[primitives/modules/concept/locator#^def-1|locator definition]]
```
