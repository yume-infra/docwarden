...
isomorph 需要一个名字表示 formatted md 中可被 semantic-lint 直接消费的 token role。

这个名字用于避免把 naming、kind、signal、policy modal 和 address token 的消费规则塞回各自 concept，导致单个 md module 承担过多职责。

## Definition

magic-word 是 formatted md 中由 isomorph 稳定、能影响 semantic-lint 读取、判断或 confidence 的 token role。 ^def-1
