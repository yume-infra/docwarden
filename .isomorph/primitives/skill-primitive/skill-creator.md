...
这个 primitive 要求 agent 在 materialize `SKILL.md`、repo-skill 或 plugin-distributed skill 前，先证明 repeatable drift、trigger boundary、soft/hard boundary、export shape 与 validation surface 都已经成立。

Pressure Scenarios:

- 用户要求 create / update skill，agent 先生成文件，却没有命名这个 skill 要防止的 agent failure。
- 用户要求从 isomorph primitive material 派生 skill，agent 却从 Codex runtime folders、plugin export shape 或 generic scaffolding 反推。
...
- skill draft 通过结构校验，但仍缺少 trigger boundary、workflow review move、pressure scenario 或 forward-test surface。

## Trigger

Description: 用于创建或更新 Codex skill、把 isomorph skill-primitive material 派生成 SKILL.md / repo-skill / plugin skill，或修正正在漂移成 generic scaffolding、CLI wrapper、膨胀 docs、trigger 不清、缺少 review gate、未验证 artifact 的 skill。
...
- 判断哪里需要 user review，并把 review unit 保持到最小 semantic unit。
...
- `isomorph primitive skill` 负责在 exportable 前验证 skill-primitive material。
...
- 先接管 relevant isomorph skill-primitive material，并把它作为 semantic source。
...
- 写入 long-term skill material 前，先 review 最小不确定 semantic unit。
...
- SKILL.md body 必须保留 capability、trigger、soft boundary、hard boundary、workflow、resource rules、review gates 与 validation。
...
- 运行 `apps/isomorph/dist/index.js primitive skill .isomorph/primitives/skill-primitive/skill-creator.md --json`，要求 `status: ready`。
...
- 当前修复吸收了旧 `primitives/modules/skill-primitive/skill-creator.md` 的 contexta 语义增量；canonical primitive path 是 `.isomorph/primitives/skill-primitive/skill-creator.md`。
