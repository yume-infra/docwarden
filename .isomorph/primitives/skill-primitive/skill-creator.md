...
创建或更新 Codex native skill，把 skill 先建模成 agent behavior intervention，并要求 agent 在 materialize 文件前证明 repeatable drift。
...
Description: 用于创建或更新 Codex skill、把 isomorph skill-primitive material 转成 skill，或修正正在漂移成 generic scaffolding、CLI wrapper、膨胀 docs、trigger 不清、缺少 review gate、未验证 SKILL.md/resources 的 skill。
...
- 判断哪里需要 user review，并把 review unit 保持到最小。
- 判断生成内容是否过于 generic、verbose 或 implementation-shaped。
...
- SKILL.md body 必须保留 capability、trigger、soft boundary、hard boundary、workflow、resource rules 与 validation。
