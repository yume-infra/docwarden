...
root `.isomorph` 只拥有 `skill-primitive` 这个 bootstrap concept 和 boundary，不拥有具体 skill 的 canonical material。
...
v0 可以从 semantic material 产出 `codex-skill` export draft，但不把 root `.isomorph/primitives` 当作最终 `SKILL.md` artifact 或具体 skill source。
